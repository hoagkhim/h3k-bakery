...
function ImportInventoryModal({ inventory, setInventory, onClose, currentUser }) {
    ...
    const handleSave = () => {
        ...
        alert(`Đã lưu phiếu nhập thành công!\nTổng tiền: ${totalAmount.toLocaleString()}đ`); onClose();
    };
            ...
            <div className="bg-white rounded-3xl w-full max-w-4xl flex flex-col shadow-2xl overflow-hidden max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto flex-1 bg-gray-50/50 flex flex-col gap-6">
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div><label className="block text-sm font-bold text-gray-700 mb-2">Ghi chú phiếu nhập</label><input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder="VD: Nhập hàng đầu tuần..." className="w-full border border-amber-200 bg-white rounded-xl p-3 focus:border-amber-500 outline-none" /></div>
                    </div>
                    <div>
...
function ExportInventoryModal({ inventory, setInventory, products, setProducts, orders, setOrders, onClose }) {
...
// --- InventoryView ---
export default function InventoryView({ inventory, setInventory, products, setProducts, currentUser, orders, setOrders }) {
    ...
    return (
        <div className="bg-white rounded-3xl shadow-sm border border-amber-100 overflow-hidden h-full flex flex-col relative">
            <div className="p-5 border-b border-amber-100 flex justify-between items-center bg-amber-50/50 shrink-0">
                <div><h3 className="font-bold text-xl text-amber-950">Quản lý Kho Nguyên Liệu</h3><p className="text-sm text-amber-600 mt-1">Theo dõi tồn kho, cảnh báo an toàn</p></div>
                <div className="flex gap-3 items-center">
                    <div className="relative"><Search className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-amber-500" /><input type="text" placeholder="Tìm mã, tên NL..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-64 pl-9 pr-4 py-2 border border-amber-200 rounded-lg text-sm outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-100 bg-white" /></div>
            ...
            {showImport && <ImportInventoryModal inventory={inventory} setInventory={setInventory} onClose={() => setShowImport(false)} currentUser={currentUser} />}
            {showExport && <ExportInventoryModal inventory={inventory} setInventory={setInventory} products={products} setProducts={setProducts} orders={orders} setOrders={setOrders} onClose={() => setShowExport(false)} />}
        </div>
    );
}
